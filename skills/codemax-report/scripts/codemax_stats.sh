***REMOVED***!/bin/bash
***REMOVED*** ============================================================================
***REMOVED*** codemax_stats.sh — CodeMax 平台（NewAPI）统计 API 封装
***REMOVED*** 用法:
***REMOVED***   bash codemax_stats.sh summary    [-s 2026-08-01] [-e 2026-08-28]
***REMOVED***   bash codemax_stats.sh users      [-s ...] [-e ...]
***REMOVED***   bash codemax_stats.sh models     [-s ...] [-e ...]
***REMOVED***   bash codemax_stats.sh pm-options
***REMOVED***   bash codemax_stats.sh pm-projects [--sort-cost] [-s ...] [-e ...]
***REMOVED***   bash codemax_stats.sh log-stat   [-s 2026-08-01] [-e ...] [-t 0]   ***REMOVED*** NewAPI 日志统计
***REMOVED***   bash codemax_stats.sh api-models                                   ***REMOVED*** NewAPI 模型清单
***REMOVED*** 认证（二选一）:
***REMOVED***   A. CODEMAX_TOKEN + CODEMAX_USER  → Bearer 令牌（系统访问令牌 + 用户 ID）
***REMOVED***   B. CODEMAX_COOKIE                → 浏览器会话 Cookie
***REMOVED***   存 settings.local.json env（CODEMAX_TOKEN=... CODEMAX_USER=... CODEMAX_COOKIE=...）
***REMOVED*** 安全: 凭证走环境变量不进 ps；URL 为内网只读统计接口
***REMOVED*** ============================================================================
set -euo pipefail

BASE="https://your-codemax.example.com"
***REMOVED*** 认证优先级: CODEMAX_ADMIN_TOKEN（admin, user=1）> CODEMAX_TOKEN+CODEMAX_USER（个人）> CODEMAX_COOKIE
