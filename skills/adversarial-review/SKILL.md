...
***REMOVED*** 对抗性审查引擎（Adversarial Review Engine · 行途版）
...
***REMOVED******REMOVED*** 〇、Token 纪律（对齐省 token 系列）
...
***REMOVED******REMOVED******REMOVED*** 审查深度决策
...
***REMOVED******REMOVED*** 一、行途自媒体四角色审阅模型
...
***REMOVED******REMOVED******REMOVED*** 审计员位置定位规则
...
***REMOVED******REMOVED*** 二、两轮流程

***REMOVED******REMOVED******REMOVED*** Round 1：独立并行审查（3-4 角色并行，精简输出）

每个角色收到相同的审查对象 + 角色定义，独立产出发现列表。

**输出格式**（每个发现）：
```markdown
***REMOVED******REMOVED******REMOVED*** [角色] 发现 ***REMOVED***N: {一句话摘要}
- 位置: {封面/正文§章节/标题}
- 严重度: 🔴 严重 / 🟡 需关注 / 🟢 建议
- 证据: {具体引用或数据}
- 理由: {为什么这是问题}
...
***REMOVED******REMOVED******REMOVED*** Round 2：脚本交叉验证 + Filter 误报过滤
...
***REMOVED******REMOVED*** 三、行途内容审查角色要点（豆包工作优化版新增）

***REMOVED******REMOVED******REMOVED*** 怀疑论者检查要点（文章/封面）
...
***REMOVED******REMOVED******REMOVED*** 领域专家（按需激活）
| 场景 | 专家 | 检查什么 |
|------|------|---------|
...
***REMOVED******REMOVED*** 四、执行指令（给 AI Agent 的标准提示词）
...
***REMOVED******REMOVED*** 五、审查报告模板

```markdown
***REMOVED*** [审查对象] 对抗性审查报告

> Round 1: 3-4 角色并行 | Round 2: 脚本交叉 | 修复: N 项 | 接受: M 项

***REMOVED******REMOVED*** 交叉验证发现（≥2 角色独立发现）

| ***REMOVED*** | 发现 | 严重度 | 报告者 |
|---|------|:--:|------|
| 1 | xxx | 🔴 | Refuter + Auditor |

***REMOVED******REMOVED*** 全部发现

| ***REMOVED*** | 发现 | 角色 | 置信度 | 严重度 | 状态 |
|---|------|------|:--:|:--:|:--:|
| 1 | xxx | Refuter | ✅ 交叉验证 | 🔴 | ✅ 已修复 |

***REMOVED******REMOVED*** 过滤统计
...
***REMOVED******REMOVED*** 七、与其他 Skill 的关系
